...
app = FastAPI(title="PAAS Face Liveness API", version="3.0-fastapi")
...
async def homepage():
    return "<html><body><h3>PAAS Face Liveness API</h3><p>FastAPI server is running.</p></body></html>"
...
async def receive_face(request: Request, face: UploadFile = File(...)):
    ...
    return JSONResponse(build_face_liveness_string_response(answer))


@app.post('/face_liveness_base64')
async def receive_face_base64(request: Request):
    ...
    answer = res['face_liveness']
    return JSONResponse(build_face_liveness_object_response(answer))


@app.post('/face_liveness_base64_batch')
async def receive_face_base64_batch(request: Request):
    data = await request.json()
